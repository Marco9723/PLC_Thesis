...
class PARCnet:
    ...
    def __init__(self,
                 ...
                 num_valid_ar_packets: int,   # sarebbe 10 ma fatto con 7
        ...
        self.packet_dim = packet_dim        # 320        NB: fadeout e fade_out
        ...
        self.neural_net = PLCModel.load_from_checkpoint(model_checkpoint, channels=1, lite=True).to(device)  # dopo format va, dovrebbe essere giusto
    ...
    def __call__(self, input_signal: np.ndarray, trace: np.ndarray, **kwargs) -> np.ndarray:
        ...
        for i, loss in tqdm(enumerate(trace), total=len(trace)):
            if loss:
                ...
                # AR model context
                valid_ar_packets = output_signal[idx - self.ar_context_len:idx]   # 7 x 320, prima faceva 10
                ...
                # ATTENZIONE QUA, NON PROPRIO COME DATASET
                nn_context = np.pad(nn_context, (0, self.pred_dim + self.extra_dim))   # nel nn paddi per 2 packets da 320 (400 + 240)
                ...
                with torch.no_grad():
                    ...
                    nn_pred = torch.istft(nn_pred, 960, 480, window=self.hann)  # 2240 + 320 + 320 = 2880   torch.Size([1, 2880])
                    nn_pred = nn_pred[..., -(self.pred_dim + self.extra_dim):]  # prendi ultimi 320 + 320 
                    nn_pred = nn_pred[..., :self.pred_dim]   # primi 400 (320 + 80)   torch.Size([1, 400])
