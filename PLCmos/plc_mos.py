...
def run_with_defaults(degraded, clean, allow_set_size_difference=False, progress=False, model_ver=1, sr=16000):  # non lo usi...
    import librosa
    ...
    for degraded_name, clean_name in iter:
        ...
        # ----------------------------#
        # Replaced sf.read with librosa.load to control the sampling rate of the signals
        # ----------------------------#

        audio_degraded, sr_degraded = librosa.load(degraded_name, sr=sr)
        audio_clean, sr_clean = librosa.load(clean_name, sr=sr)
        # audio_degraded, sr_degraded = sf.read(degraded_name)
        # audio_clean, sr_clean = sf.read(clean_name)
        # assert sr_degraded == sr
        # assert sr_clean == sr
...
if __name__ == "__main__":
    ...
    parser.add_argument("--allow-set-size-difference", type=bool, default=False,
                        help="Set to true to allow the number of degraded and clean audio files to be different")

    # NEW #
    parser.add_argument("--sr", type=int, default=16000, help="The desired sampling rate of the audio signals")
